...
from utils.img_utils import shave_a2b, resize_tensor_w_kernel, create_penalty_mask


class DownSampleRegularization(nn.Module):
    lambda_sum2one = 0.5
    lambda_bicubic = 5
    ...
    def __init__(self, scale_factor, G_kernel_size):
        super(DownSampleRegularization, self).__init__()
        ...
        # check kernel shift issue, set 1
        # self.criterion_centralized = CentralizedLoss(G_kernel_size, scale_factor=1).cuda()
...
class CentralizedLoss(nn.Module):
    ...
    def forward(self, kernel):
        ...
        kernel = shave_a2b(kernel.unsqueeze(0).unsqueeze(0), self.mask.unsqueeze(0).unsqueeze(0))
