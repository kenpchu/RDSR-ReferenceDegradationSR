...
from networks1 import Generator_UP_DRCA_V2, Generator_DN_DRCA_V2, weights_init_G_R_DN, weights_init_G_R_UP
...
class DualSR:
    ...
    def __init__(self, conf, test_loader=None, encoder_network=None):
        ...
        if self.conf.with_dr:
            ...
            self.G_R_DN = Generator_DN_DRCA_V2(self.G_DN).cuda()
            self.G_R_UP = Generator_UP_DRCA_V2(self.G_UP, scale_factor=self.conf.scale_factor).cuda()
        ...
        if self.conf.with_dr:
            self.G_R_DN.apply(weights_init_G_R_DN)
            self.G_R_UP.apply(weights_init_G_R_UP)
    ...
    def eval_whole_img(self):
        ...
        if (self.iter > self.conf.start_min_loss_iter and self.iter % (self.conf.eval_iters * 5) == 0) or self.iter == self.conf.start_dr_iter:
            self.save_img(tar_rec_hr_w, f'tar_rec_hr_{self.iter}_w')
            self.save_img(tar_rec_lr_w, f'tar_rec_lr_{self.iter}_w')
            self.save_img(tar_gt_rec_lr_w, f'tar_gt_rec_lr_{self.iter}_w')
